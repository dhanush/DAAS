...
public class OrientObjectDbPoolTransactionManager extends AbstractPlatformTransactionManager implements
	...
	protected Object doGetTransaction() throws TransactionException {
		ObjectOrientTransactionObject txObject = new ObjectOrientTransactionObject();
		return txObject;
	...
	protected void doBegin(Object transactionObject, TransactionDefinition definition) throws TransactionException {
		...
		ObjectOrientTransactionObject txObject = (ObjectOrientTransactionObject) transactionObject;
		ODatabaseObject db = orientDbTemplate.getObjectDatabase();
		...
		try {
			...
			if (!TransactionSynchronizationManager.hasResource(ODatabase.class))
				TransactionSynchronizationManager.bindResource(ODatabase.class, txObject.getDatabaseHolder());
			...
			throw new OrientDbTransactionException("Couldnt open a Transaction with Object: " + txObject, e);
		}

	}

	protected void closeDatabaseConnectionAfterFailedBegin(OrientTransactionObject txObject) {
		ODatabaseObject db = txObject.getDatabaseHolder().getObjectDatabase();
		try {
	...
	protected void doCommit(DefaultTransactionStatus status) throws TransactionException {
		...
		ObjectOrientTransactionObject txObject = (ObjectOrientTransactionObject) status.getTransaction();
		if (status.isDebug()) {
			logger.debug("Committing transaction on DB [" + txObject.getDatabaseHolder().getObjectDatabase() + "]");
		}
		try {
			ODatabaseObject db = txObject.getDatabaseHolder().getObjectDatabase();
			db.commit();
	...
	protected void doRollback(DefaultTransactionStatus status) throws TransactionException {
		...
		ObjectOrientTransactionObject txObject = (ObjectOrientTransactionObject) status.getTransaction();
		if (status.isDebug()) {
			logger.debug("Rolling back OrientDB transaction on DB [" + txObject.getDatabaseHolder().getObjectDatabase()
					+ "]");
		}
		try {
			ODatabaseObject db = txObject.getDatabaseHolder().getObjectDatabase();
	...
	protected void releaseDatabase(ODatabaseHolder holder) {
		...
		System.out.println("release conn :" + holder.getObjectDatabase().hashCode()); 
