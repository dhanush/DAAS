...
public class AccountDaoImpl extends AbstractDao<Account> implements AccountDao {
	...
	@Autowired
	private OrientDbConnectionManager dbConnectionManager;
	...
	@Override
	public List<Account> list() throws BaasPersistentException, BaasEntityNotFoundException {
		ODatabaseObject db = getObjectDataBase();
		try {
	...
	public boolean findAny(Map<String, String> propertyToValue) throws BaasPersistentException {
		...
		try {
			...
			long count = ((ODocument) db.query(new OSQLSynchQuery<Account>(sql)).get(0))
					.field("count");
